...
Native Node.js projects are installed with `npm ci`, then the dashboard previously
required an npm script named `build`. Server-rendered Express applications often
have no compilation step and expose only `start`; forcing a fictitious `build`
script prevents an otherwise valid deployment.
...
performs `npm ci`, skips the build phase, then starts and health-checks the
application. A supplied build script remains a constrained npm-script name;
free-form shell commands are still prohibited.
