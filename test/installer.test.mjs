...
test('helper keeps Docker Compose project activation bounded to guarded policy checks', async () => {
  ...
  assert.match(helper, /hostmgr-unmatched\.conf/);
...
test('Python remains optional while project execution drops privileges before any Python command', async () => {
  ...
  assert.match(helper, /writeFile\('\/etc\/mailname', `\$\{mail\.hostname\}\\n`, \{ mode: 0o644 \}\)/);
  assert.ok(helper.indexOf("writeFile('/etc/mailname'") < helper.indexOf('await writeMailMaps(mail)'));
